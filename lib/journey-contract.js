...
class JourneyContract extends Contract {
     ...
     *
     * @param {*} ctx
     * @param { (string|JourneyKey) } key The key via `createKey()` or an object for producing the key
     *
    ...
    createKey(ctx, journeyKey) {
        const keyAttrs = [journeyKey.droneId, journeyKey.owner, journeyKey.type, journeyKey.status];
