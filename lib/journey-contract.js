...
class JourneyContract extends Contract {
    ...
    /**
     * @param {string} journeyKey - The journey key in JSON
     * @param {string} newState - JSON A partial Journey
     *
     * @return {Promise<Buffer>}
     */
    async updateJourney(ctx, journeyKey, newState) {
        const state_o = JSON.parse(journeyKey);
        ...
        return await this._saveJourney(ctx, journeyId, JSON.stringify({ ...query, ...newState_o }));
    }
